...
- To keep overlapping paths readable, the RViz visualization offsets only the displayed paths: the target path is shifted slightly in `+Y`, and the actual trail is shifted slightly in `-Y`. The commanded target positions and FK positions are not changed.
